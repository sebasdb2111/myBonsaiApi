...
class UserController extends Controller
{
    ...
    public function editAction(Request $request)
    {
        ...
        if ($apellidos != null) $user->setApellidos($apellidos);
        if ($fechaNacimiento != null) $user->setFechanacimiento(new \DateTime($fechaNacimiento));
        if ($imgUser != null) $user->setImguser($imgUser);
        if ($password != null){
            $pwd = hash('SHA256',$password);
            $user->setPassword($pwd);
        }
